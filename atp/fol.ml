...
let rec print_term curry formater prec fm =
  match fm with
    Var x -> pp_print_string formater x
  | Fn("^",[tm1;tm2]) -> print_infix_term curry formater true prec 22 "^" tm1 tm2
  | Fn("*",[tm1;tm2]) -> print_infix_term curry formater false prec 20 "*" tm1 tm2
  | Fn("-",[tm1;tm2]) -> print_infix_term curry formater true prec 18 "-" tm1 tm2
  | Fn("+",[tm1;tm2]) -> print_infix_term curry formater false prec 16 "+" tm1 tm2
  | Fn("::",[tm1;tm2]) -> print_infix_term curry formater false prec 14 "::" tm1 tm2
  | Fn(f,args) -> print_fargs curry formater f args  

and print_fargs curry formater f args = 
  let print_args sep =
    open_box 0;
    print_term curry formater 0 (hd args); 
    (* pp_print_break formater 0 0; *)
    do_list (fun t -> 
	       pp_print_string formater sep; 
	       (* pp_print_break formater 0 0; *)
	       print_term curry formater 0 t)
      (tl args);
    close_box()
  in    
  if args = [] then 
    pp_print_string formater f
  else
    if not curry then 
      begin
	pp_print_string formater f;
	(pp_print_string formater "(";
	 print_args ",";
	 pp_print_string formater ")")
      end
    else
...
and print_infix_term curry formater isleft oldprec newprec sym p q =
  if oldprec > newprec then (pp_print_string formater "("; open_box 0) else ();
  print_term curry formater (if isleft then newprec else newprec+1) p;
  pp_print_string formater (" "^sym); pp_print_space formater ();
  print_term curry formater (if isleft then newprec+1 else newprec) q;
  if oldprec > newprec then (close_box(); pp_print_string formater ")") else ();;

let printert curry formater fm = open_box 0; print_term curry formater 0 fm; close_box();;
...
let print_atom curry formater prec (R(p,args)) =
  if mem p ["="; "<"; "<="; ">"; ">="] & length args = 2
  then print_infix_term curry formater false 12 12 p (el 0 args) (el 1 args)
  else print_fargs curry formater p args;;

let printer ?curryfied:(curry=false) = formula_printer std_formatter (print_atom curry std_formatter);;
let string_printer ?curryfied:(curry=false) = formula_printer str_formatter (print_atom curry str_formatter);;
