...
class type t_objet_segment_deux_points =
object
...
class objet_segment_deux_points p1 p2 =
object (self)
  ...
  method dessine win rep =
    ...
    let l = param_segment_inter_cadre c1 c2 in
      dessiner_segment_parametres win rep l eq

  method def_langue_naturelle_corps () =
    self#objet_nom^" be the segment ["^p1#objet_nom^","^p2#objet_nom^"]"
  ...
  method output_eukleides_body () = 
    sprintf "%s = segment(%s,%s)" self#objet_nom p1#objet_nom p2#objet_nom
