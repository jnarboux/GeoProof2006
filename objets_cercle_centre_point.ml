...
class type t_objet_cercle_centre_point =
  object
...
class objet_cercle_centre_point p1 p2 =
  object (self)
    inherit objet_cercle
	
    method def_langue_naturelle_corps () = 
      self#objet_nom^" be the circle of center "^p1#objet_nom^" going through "^p2#objet_nom
	...
	and c2 = p2#coordonnees_point in
	  self#change_parametres
	    (EQ_CERCLE (equation_circle_center_point c1 c2));
    ...
    method output_drg_body () =
      Xml.Element("CircleCenterPoint",
		  [  
		     ("name",self#objet_nom); 
		     ("center",p1#objet_nom);
		     ("through",p2#objet_nom)
		 ],[])
	...
	| Guilhot -> None
	    (*
	    Some(Atom(R("cercle_rayon",[Var(self#objet_nom);Var(p1#objet_nom);Var(p2#objet_nom)])))
	    *)

    method output_pst_eucl_body () = ""

    method output_eukleides_body () = 
      sprintf "%s = circle(%s,distance(%s,%s))" self#objet_nom p1#objet_nom p1#objet_nom p2#objet_nom
	
    method predicate_form () = Some(True
(*Not(Atom(R("=",[Var(p1#objet_nom);Var(p2#objet_nom)]))) *))
