...
class type t_object_test_between =
object
...
class object_test_between coordsinit p1 p2 p3 =
object (self)
  inherit objet_texte
  ...
  method recalcule =
    ...
    if (between c1 c2 c3) then 
      self#set_text (i18n "Between") else 
      self#set_text (i18n "Not between")
 ...
 method predicate_form () = Some (Atom(R("between",[
					  Var(p1#objet_nom);
					  Var(p2#objet_nom);
					  Var(p3#objet_nom);
					  ])))
