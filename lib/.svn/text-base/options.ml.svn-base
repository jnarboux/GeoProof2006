...
let file_section file section_name section_help =
  let s =
    {section_name = section_name; section_help = section_help;
     section_file = file; section_options = []}
  in
  file.file_sections <- file.file_sections @ [s]; s
  
let create_options_file name =
  let file =
    {file_name = name; file_sections = []; file_rc = []; file_pruned = false}
  in
...
let define_simple_option
  ...
  let o =
    {option_name = option_name; option_help = option_help;
     ...
     option_default = default_value; option_hooks = [];
     option_section = section; option_advanced = advanced; option_desc = desc}
  in
...
let define_option opfile option_name =
  fun ?desc option_help option_class default_value ->
    define_simple_option true opfile option_name desc option_help option_class
      default_value false

let define_expert_option opfile option_name =
  fun ?desc option_help option_class default_value ->
    define_simple_option true opfile option_name desc option_help option_class
      default_value true
...
and parse_id (strm__ : _ Stream.t) =
  ...
  | _ -> raise Stream.Failure
and parse_list list (strm__ : _ Stream.t) =
  match Stream.peek strm__ with
    Some (Kwd ";") -> Stream.junk strm__; parse_list list strm__
  | Some (Kwd ",") -> Stream.junk strm__; parse_list list strm__
  | Some (Kwd ".") -> Stream.junk strm__; parse_list list strm__
  | _ ->
      match
        try Some (parse_option strm__) with
          Stream.Failure -> None
      with
        Some v -> parse_list (v :: list) strm__
      | _ ->
          match Stream.peek strm__ with
            Some (Kwd "]") -> Stream.junk strm__; List.rev list
          | Some (Kwd ")") -> Stream.junk strm__; List.rev list
          | _ -> raise Stream.Failure
      ...
      try
        ...
        let list =
          try parse_gwmlrc stream with
            e ->
              Printf.eprintf
                "Syntax error while parsing file %s at pos %d:(%s)\n" filename
                (Stream.count s) (Printexc2.to_string e);
              exit 2
        in
        Hashtbl.clear once_values;
        let affect_option o =
          ...
          | e ->
              ...
              exit 2
        in
        List.iter (fun s -> List.iter affect_option s.section_options)
          sections;
        close_in ic;
        list
      with
        e -> lprintf "Error %s in %s\n" (Printexc2.to_string e) filename; []
    with
...
let load opfile =
  try opfile.file_rc <- really_load opfile.file_name opfile.file_sections with
    ...
    if !advanced then
      begin
        ...
        List.iter
          (fun s ->
             let options =
               List.filter (fun o -> o.option_advanced) s.section_options
             in
             if options = [] then ()
             else
               let _ = () in
...
let get_option opfile name =
  let rec iter name list sections =
    match list with
      o :: list -> if o.option_name = name then o else iter name list sections
    | [] ->
        match sections with
          [] ->
            prerr_endline
...
let simple_options prefix opfile =
  ...
  List.iter
    (fun s ->
       List.iter
         (fun o ->
            try list := strings_of_option prefix o :: !list with
              _ -> ())
         s.section_options)
    opfile.file_sections;
  List.rev !list
