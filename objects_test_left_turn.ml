...
class type t_object_test_left_turn =
object
...
class object_test_left_turn coordsinit p1 p2 p3 =
object (self)
  inherit objet_texte
  ...
  method recalcule =
    ...
    if (left_turn c1 c2 c3) then 
      self#set_text (i18n "Left turn") else 
      self#set_text (i18n "Not left turn")
  ...
  method predicate_form () = Some (Atom(R("left_turn",[
					  Var(p1#objet_nom);
					  Var(p2#objet_nom);
					  Var(p3#objet_nom);
					  ])))
